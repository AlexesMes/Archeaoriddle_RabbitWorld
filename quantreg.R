...
fit_rq <- rq(earliest ~ dist_org, tau = 0.99, data=f_siteInfo, alpha=0.95)

# Derive overall estimated rate of dispersal
-1/summary(fit_rq)$coefficients[2,] 
#Note: Between -5.56*10^(-309) to 5.56*10^(-309) km/year, with estimate at 2.42 km/year. 'Lower bd' and 'upper bd' represent the endpoints of confidence intervals for the model coefficients
...
runFun <- function(seed, dat, theta_init, constants, nburnin, thin, niter)
{
  ...
  model <- nimbleCode({
    ...
    #priors 
    alpha ~ dnorm(8000, sd=200); #beta_0 #Assume the first migration to be somewhere between 8200BP and 7800BP. Note: age of approximate origin, Farwallow: 8038 cal BP.
...
seeds  <-  c(12, 34, 56, 78)
niter  <- 50#6000000 #Number of iterations #Working 1000000 and 500000
nburnin  <- 10#3000000 #Burn-in iterations (after this assuming chain has reached a good approximation of stationary)
thin  <- 3 #Parameters sampled every 300 steps
